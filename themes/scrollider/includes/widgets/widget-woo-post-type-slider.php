...
class Woo_Widget_Slider extends WP_Widget {
	...
	public function widget( $args, $instance ) {
		...
		while ( $query->have_posts() ) { $query->the_post();
			...
			$html .=  '<li class="slide">' . "\n";
			$woo_image = woo_image( 'width=500&class=thumbnail&noheight=true&return=true&single=true&link=img' );
			if ( $woo_image == '' ) {
				$noimage = ' no-image';
			} else {
				$noimage = '';
			...
			if ( 'slide' == get_post_type( $post->ID ) && get_post_meta($post->ID, 'url', true) != '' )
				$url = get_post_meta($post->ID, 'url', true);

			if ( $woo_image != '' ) {
				$html .= '<a href="' . esc_url( $url ) . '" class="widget-slider-title" title="' . esc_attr( get_the_title() ) . '">';
				$html .= $woo_image;
				$html .= '</a>';
			}
			...
			$html .= $title . "\n";
			$html .= '<a href="' . esc_url( $url ) . '" class="widget-slider-title" title="' . esc_attr( get_the_title() ) . '">' . get_the_title() . '</a>' . "\n";
